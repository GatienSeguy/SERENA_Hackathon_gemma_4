...
{\normalsize Gatien S\'eguy$^{1}$ \quad Matys Savidan$^{1}$}
\vspace{2mm}

{\small $^{1}$ENS Paris-Saclay}
\vspace{2mm}
...
\begin{tikzpicture}[
    ...
    \draw[arrow] (user) -- (p1);
    \draw[arrow] (p1) -- node[above, label] {\tiny JSON} (router);
    \draw[arrow] (router) -- node[above, label] {\tiny action} (p2);
    \draw[arrow] (p2) -- (reply);
