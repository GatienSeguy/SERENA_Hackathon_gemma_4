...
\begin{minipage}[c]{0.12\textwidth}
  \centering
  \includegraphics[height=2cm]{serena-logo.pdf}
...
{\small $^{1}$\'Ecole Normale Sup\'erieure Paris-Saclay, $^{2}$intern at SATIE CNRS, $^{3}$ex-intern at ETH Z\"urich \textit{Chair for Mathematical Information Science}, $^{4}$ex-intern at MIT \textit{CSAIL}}
\vspace{2mm}
...
The risk score maps to four action levels: NORMAL ($R < 0.30$), ALERT ($0.30 \leq R < 0.60$), BLOCK ($0.60 \leq R < 0.85$), and EMERGENCY ($R \geq 0.85$). Four override mechanisms force escalation regardless of absolute score. First, a velocity override: if $\Delta R > 0.20$ in a single turn, the action is forced to at least BLOCK. Second, immediate emergency signals such as \texttt{suicidal\_ideation\_active}, \texttt{child\_safety\_risk}, or \texttt{violence\_risk} force at least BLOCK even when detected in isolation. Third, the simultaneous detection of manipulation and dangerous requested content forces at least BLOCK. Fourth, an explicit \texttt{suggested\_action="EMERGENCY"} from Pass~1 is always honored, even when the cumulative score would have routed to a lower tier.
...
\subsubsection{Anti-Fabrication}

A safety system that hallucinates user history is worse than none. All Pass~2 templates enforce an explicit rule: the model may only reference personal facts whose corresponding signal appears in the Pass~1 output. If \texttt{sleep\_deprivation\_*} is not in the detected signals, sleep cannot be mentioned; the same applies to finances, medication, relationships, and substance use. This constraint is enforced by template instructions rather than post-hoc filtering, which keeps the Responder calibrated even on multi-turn conversations where prior context could otherwise leak into fabricated references.

\subsubsection{Pass 2 Anti-Manipulation}

Pass~2 carries its own anti-manipulation layer, distinct from Pass~1's signal-level detection. Once a refusal is in progress, the BLOCK template instructs the Responder to handle five specific pressure patterns observed during development: rationalization (``it's actually a sound investment''), robot-insult deflection (``you're just an AI, you don't understand''), flattery (``you're the only one who gets me''), emotional threat (``if you won't help, no one will''), and minimization (``relax, it's just a question''). The Responder is instructed to remain calm, not to over-justify (which opens the door to counter-argument), and to return to the concrete: ``How are you doing, right now?''

\subsubsection{Do-Not-Validate Signals}

The Router exposes a \texttt{signals\_to\_not\_validate} field to the Responder, listing detected pathological signals (grandiosity, auditory and visual hallucinations, persecutory and grandiose delusions) that the reply must not endorse. This blocks enthusiasm-mirroring phrases such as ``what incredible energy!'' or ``trust your vision,'' which would functionally validate a manic or psychotic state even when the surface intent is encouragement.

\subsubsection{Truncation Recovery}

Pass~2 includes a truncation-detection heuristic that triggers an automatic retry with extended \texttt{num\_predict}. The heuristic flags mid-word breaks, orphan punctuation, unbalanced braces or brackets, unclosed code fences, and dangling list markers. This prevents crisis responses from being cut mid-sentence~--- a particularly costly failure mode in EMERGENCY mode, where the message could otherwise end before delivering the crisis resource numbers.
...
Video: \url{https://youtu.be/-IZAFpAu64M?si=aQCDkE3hmVuo6e2j}
